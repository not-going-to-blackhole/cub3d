...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/19 16:05:18 by woorikim          #+#    #+#             */
/*   Updated: 2024/03/19 16:07:55 by woorikim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_wall(t_game *game, t_ray *ray)
{
	if (ray->side == WALL_HORIZ)
		ray->perp_wall_dist = (ray->hit_x - game->player->pos_x + \
			(1 - ray->step_x) / 2) / ray->ray_dir_x;
	else
		ray->perp_wall_dist = (ray->hit_y - game->player->pos_y + \
...
void	paint_wall(t_game *game, t_ray *ray, t_render *render, int x)
{
	...
	render->pos = (ray->wall_start - WIN_HEIGHT / 2 + ray->wall_height / 2) * \
		render->step;
	while (y < ray->wall_end)
	{
		render->text_y = (int)render->pos & (TILE_SIZE - 1);
		render->pos += render->step;
		render->color = game->texture[render->text_idx].data[TILE_SIZE * \
